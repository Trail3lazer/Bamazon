                ...
                con.query("SELECT * FROM departments", (err, table) => {
                    if (err) throw err;
                        ...
                        var table1 = new Table({
                            head: ['Name', 'Overhead Costs', "Product Sales", "Profit"]
                            , colWidths: [20, 10, 10, 10]
                        });

                        // table is an Array, so you can `push`, `unshift`, `splice` and friends
                        for (i in table) {
                            let department = table[i];
                            let depArray = [];
                            for (key in department){
                                if (key !== "id") {
                                depArray.push(department[key])
                                }
                            };
                            let profit = department['product_sales']-department['over_head_costs'];
                            depArray.push(profit);
                            table1.push(depArray);
                        };
